...
//			return $image;
	...
	public static function check() {
		if (Request::hasParameter(GD_KEY)) {
			...
			$cacheHash = self::getCacheHash($filename, $width, $height);
			$filepath = self::getCacheFilePath($cacheHash);
			if (self::isCached($cacheHash)) {
			...
			$image = self::normalize($filename, $width, $height, $outputMode, $filepath);
	...
	public static function isCached($cacheHash) {
		$filepath = self::getCacheFilePath($cacheHash);
		return file_exists($filepath);
	}

	public static function getCachePath() {
		return Config::get('GD_CACHE');
	}

	public static function getCacheFilePath($cacheHash) {
		return self::getCachePath() . DIRECTORY_SEPARATOR . "$cacheHash.jpg";
	}

	public static function getCacheHash($filename, $width = false, $height = false) {
		
		$size = (file_exists($filename)) ? filesize($filename) : 0;
		
		return sha1($filename . "_$width-$height-$size");
	}
	...
	public function getFinalSrc($pathURI, $width, $height, $outputMode = 'landscape') {

		$realPath = GD_BASE_PATH . $pathURI;
		if (file_exists($realPath)) {
			return self::mountSrc($pathURI, $width, $height, $outputMode = 'landscape');
		} else {
			return self::getDefaultSrc($width, $height, $outputMode = 'landscape');
		}
	}
